...
export default function  HomeAppliances(){
    ...
    let homeAppliancesCategory = productsData.filter((row)=>{
        if(row.category=="Home Appliances"){
            return row;
        }
    });
    return(
        ...
        <Container>
            ...
            <Grid container className={classes.root} spacing={2}>
                <Grid item xs={12} >
                    <Grid container spacing={spacing}>
                        ...
                        {homeAppliancesCategory.slice(0,visible).map((product) => {
                            return(
            ...
            {
                visible+1<=homeAppliancesCategory.length?
